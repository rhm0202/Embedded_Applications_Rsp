...
Y1_HI = np.array([35, 255, 255], np.uint8)
...
def detect_maskY_BGR(frame):
    ...
    # need to tune params
    Y = G * 0.5 + R * 0.5 - B * 0.7   # 연산 수행 시 float64로 변경됨
    Y = Y.astype(np.uint8)
...
def detect_maskY_HSV(frame):
    ...
    # need to tune params
    mask_Y = cv.inRange(crop_hsv, Y1_LO, Y1_HI)
    return mask_Y
...
def main():
    ...
    try:
        while( camera.isOpened() ):
            ...
            if save_flag and (current_time - last_save_time > save_interval):
                # 캡처: line_tracing이 설정한 angle_to_save 값 사용
                save_img(crop_img, angle_to_save)
